...
if [ "${DING_TOKEN}" != "" ]; then
    ...
    output_info="Build [#$TRAVIS_BUILD_NUMBER]($TRAVIS_BUILD_WEB_URL)($TRAVIS_COMMIT) of $TRAVIS_REPO_SLUG@$TRAVIS_BRANCH by $author $TRAVIS_SD_STATUS in $elapsed_seconds seconds"
